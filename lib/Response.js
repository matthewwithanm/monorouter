...
Response.prototype.setState = function(state, cb) {
  // The Response's state is basically a window onto the router state, since
  // we don't want to accidentally clobber `view`. This assumes that you only
  // ever change router state through the route context.
  var newState = {viewProps: merge(this.state.viewProps, state)};
  this.router.setState(newState, function() {
    this.state = this.router.state.viewProps;
    if (cb) cb();
  }.bind(this));
};

Response.prototype.setView = function(view, cb) {
  this.router.setState({view: view}, function() {
    if (cb) cb();
  }.bind(this));
};
