...
Router.prototype = {
  ...
  renderInto: function(element, opts) {
    ...
    var rerender = function() {
      if (this.renderer) {
        this.renderer.setProps({view: req.view, state: req.state});
      }
    }.bind(this);
    ...
    var update = function(isInitial) {
      ...
      var cleanup = createCleanup(this._currentResponse.request);
      this._currentResponse.request
        .on('end', cleanup)
        .on('error', cleanup);

      if (isInitial) {
        this._currentResponse.request.once('endInitial', initApp);
      }
    }.bind(this);
