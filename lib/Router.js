...
function Router(opts) {
  this.state = {};
  this.defaultVars = opts && opts.defaultVars;
}
...
Router.prototype.getVars = function() {
  return extend(this.defaultVars, this.state);
};
...
Router.prototype.dispatch = function(url, opts, callback) {
  ...
  try {
    ...
    var msg = 'URL not within router root: ' + url;
    callback(new Unhandled(new Request(url, opts), msg));
    return;
  }

  // Wrap the callback, imposing a delay to force asynchronousness in
  // case the user calls it synchronously.
  var cb = delayed(function(err) {
    // If we've exhausted the middleware without handling the request, call the
    // `unhandled()` method. Going through `unhandled` instead of throwing the
    // error here means we're always going through the same process, getting the
    // same events, etc.
    if (!err && !res.ended) return res.unhandled();
