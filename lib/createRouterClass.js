 ...
 */
function createRouterClass(viewFactory, opts) {
  ...
  var Router = React.createClass({
    ...
    render: function() {
      return viewFactory(this.state.viewProps);
    },

    dispatch: function(urlOrRequest, callback) {
      var req = urlToRequest(urlOrRequest);
      ...
      Router.dispatch(req, this.state.viewProps,
        this.handleViewPropsChange,
        cb
      );
    },
    ...
    statics: {
      ...
      dispatch: parseDispatchArgs(function(path, props, callback) {
        var req = typeof arguments[0] === 'string'
      ...
      var handled = Router.routes.some(function(route) {
        ...
        if (match) {
          req.params = match;
          var ctx = new RoutingContext(req, this);
      ...
      renderToString: parseDispatchArgs(function(path, props, callback) {
        Router.dispatch(path, props, function(err, res) {
          if (err) {
            callback(err);
            return;
          }
          ...
          var str;
          try {
            str = React.renderComponentToString(res.view());
          } catch (err) {
            callback(err);
            return;
          }
